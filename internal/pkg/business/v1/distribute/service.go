...
func (service Service) DDW(context *gin.Context, inventory string) error {
	...
	if !isMaster {
		ddwBakPath := dmConfigs["BAK_PATH"].Value + "/" + objectName + "-ddw-bak"
	...
	if !isMaster {
		...
		if s3IniExist {
			...
			*/
			...
			//执行流式备份初始化数据集到s3上
			err = service.CommonService.DmrmanExecCmd(context, "RESTORE DATABASE '"+instancePath+"/dm.ini' FROM BACKUPSET '"+dmConfigs["BAK_PATH"].Value+"/"+podName[:strings.LastIndex(podName, "-")]+"-ddw-bak' device type tape")
			if err != nil {
				klog.Errorf("DmrmanExecCmd error: %s......", err)
			}
			err = service.CommonService.DmrmanExecCmd(context, "RECOVER DATABASE '"+instancePath+"/dm.ini' FROM BACKUPSET '"+dmConfigs["BAK_PATH"].Value+"/"+podName[:strings.LastIndex(podName, "-")]+"-ddw-bak' device type tape")
			if err != nil {
				klog.Errorf("DmrmanExecCmd error: %s......", err)
			}
...
func (service Service) DDWWakeUp(context *gin.Context) error {
	ddwWait.Done()
	klog.Infof("ddwWakeUp success")
	return nil
}
...
func (service Service) Share(context *gin.Context, inventory string) error {
	...
	if s3IniExist {
		...
		*/
		...
		//执行流式备份初始化数据集到s3上
		err = service.CommonService.DmrmanExecCmd(context, "BACKUP DATABASE '"+instancePath+"/dm.ini' FULL BACKUPSET '"+dmConfigs["BAK_PATH"].Value+"/"+podName[:strings.LastIndex(podName, "-")]+"-ddw-bak'  device type tape")
		if err != nil {
			klog.Errorf("DmrmanExecCmd error: %s......", err)
		}
	...
	ddwBakJson, err := tools.ReadFile(ddwBakPath + "/" + objectName + "-ddw-bak.bak.json")
	if err != nil {
		klog.Errorf("ReadFile error: %s......", err)
	}
	ddwMetaJson, err := tools.ReadFile(ddwBakPath + "/" + objectName + "-ddw-bak.meta.json")
	if err != nil {
		klog.Errorf("ReadFile error: %s......", err)
	}
	...
	for node := 1; node < replicas; node++ {
		...
		go func(node int) {
			...
			if err != nil {
				...
				if resp.StatusCode == 200 {
					resp, err := tools.Get("http://" + monDwDomainName + "/dmctl/v1/ddwWakeup")
					if err != nil {
						klog.Errorf("get ddwWakeup err: %s", err)
					} else {
						if resp.StatusCode == 200 {
...
func (service Service) BakJsonRestore(context *gin.Context, ddwBak *typed.DdwBak) error {
	...
	ddwBakJsonFile := ddwBak.DdwBakPath + "/" + objectName + "-ddw-bak.bak.json"
	ddwMetaJsonFile := ddwBak.DdwBakPath + "/" + objectName + "-ddw-bak.meta.json"
